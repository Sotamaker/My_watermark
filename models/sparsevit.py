...
class SparseViT_Mul(nn.Module):
    ...
    def forward(self, image):
        mask_feat = self.encoder_net(image)
        feature_list = []
        for k, v in mask_feat.items():
            feature_list.append(v)
            
        pred_mask = self.lmu(feature_list)
        pred_mask = F.interpolate(pred_mask, size = (self.img_size, self.img_size), mode='bilinear', align_corners=False)
